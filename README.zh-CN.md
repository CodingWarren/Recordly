...
## 实时绘图板（Excalidraw）
在录制过程中随时打开全屏绘图覆盖层。基于 [Excalidraw](https://excalidraw.com/) 驱动，绘图板让你无需切换软件即可绘制示意图、标注界面、画流程图，所有笔触都会直接合成到录制画面中。

- **自由画笔、形状、箭头与文本** — 完整的 Excalidraw 工具集
- **深色 / 浅色主题切换**，可选网格模式
- **随时导出画布为 PNG**
- **状态持久化** — 绘图数据保存到 `.recordly` 项目文件，重新打开后自动恢复
- **零指针拖影** — 正确区分屏幕录制与窗口录制，彻底消除无限镜像效应
- **窗口录制画中画（PiP）** — 目标窗口作为实时背景显示在绘图板内，WGC 一次性捕获内容与绘图的合成画面
...
- 可在应用中打开已有录像或已有项目文件
- **录制中打开绘图板覆盖层**，使用 Excalidraw 实时标注
...
- 全屏 Excalidraw 绘图覆盖层，可在录制中随时启动
- 自由画笔、直线、箭头、矩形、椭圆、菱形与文本
- 深色与浅色主题，可选网格模式
- 导出画布快照为 PNG
- 绘图状态持久化到 `.recordly` 项目文件
- 屏幕录制：透明覆盖层 — WGC 直接捕获含绘图的完整显示器，无需背景视频
- 窗口录制：画中画 — 目标窗口作为实时背景，WGC 录制合成帧
- 绘图板打开期间隐藏系统鼠标指针，消除拖影效应
...
**绘图板**
- 可在录制中打开的全屏 Excalidraw 覆盖层
- 屏幕录制时，绘图板作为透明窗口由 WGC 随显示器一并捕获，无背景视频，无镜像循环
- 窗口录制时，目标窗口以实时画中画背景显示在绘图板内，WGC 录制合成帧
- 绘图状态序列化后存储于 `.recordly` 项目文件
- 架构细节请参阅 [`DRAWING_BOARD_DEV_NOTES.md`](DRAWING_BOARD_DEV_NOTES.md)
